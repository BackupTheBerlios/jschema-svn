...
public class TestFrame extends JFrame {	
	...
	private void process(String uri) {
		try {
			...
			HtmlRendererContext rcontext = new SimpleHtmlRendererContext(this.htmlPanel);
			DocumentBuilderImpl builder = new DocumentBuilderImpl(context, rcontext);
			HTMLDocumentImpl document = builder.parse(new InputSourceImpl(bin, uri, "ISO-8859-1"));
			long time2 = System.currentTimeMillis();
			System.out.println("Parse elapsed=" + (time2 - time1) + " ms.");
			this.tree.setModel(new NodeTreeModel(document));
			this.htmlPanel.setDocument(document, rcontext);//, new LayoutArgs());
		} catch(Exception err) {
			err.printStackTrace();
		}
	}
