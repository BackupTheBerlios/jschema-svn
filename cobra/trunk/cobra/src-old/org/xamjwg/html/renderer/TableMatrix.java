...
public class TableMatrix {
	 ...
	 * @param element
	 */
    public TableMatrix(HTMLTableElementImpl element, Container container, LayoutArgs layoutArgs) {
	...
	private void populateColumns(NodeImpl rowNode, ArrayList row, int rowIndex, int cellPadding) {
		...
		while(i.hasNext()) {
			...
			ActualCell ac = new ActualCell(columnNode, cellPadding, layoutArgs);
...
//	private void determineColumnSizesFixedTW(int border, int cellSpacingX, int cellSpacingY, int availWidth, int tableWidth) {
//
//		int hasBorder = border > 0 ? 1 : 0;
//
//		SizeInfo[] columnSizes = this.columnSizes;
...
//		// Determine tentative column widths based on specified cell widths
//		
//		this.determineTentativeSizes(columnSizes, widthsOfExtras, cellAvailWidth);
//		
//		// Pre-render cells. This will give the minimum width of each cell,
//		// in addition to the minimum height.
//		
//		this.preRender(hasBorder, cellSpacingX, cellSpacingY, false);
//
//		// Adjust column widths based on minimums of each cell.
//		
//		this.adjustForRenderWidths(columnSizes, hasBorder, cellSpacingX);
//		
//		// Adjust for expected total width
//		
//		int totalUsedInCells = this.adjustSizesForExpectedMax(columnSizes, cellAvailWidth, widthKnown);
//		
//		// Calculate actual table width
//
//		this.tableWidth = totalUsedInCells + widthsOfExtras;
...
//	
//
//	private void determineColumnSizesFlexibleTW(int border, int cellSpacing, int availWidth) {
//
//		int hasBorder = border > 0 ? 1 : 0;
//
//		SizeInfo[] columnSizes = this.columnSizes;
...
//		int widthsOfExtras = this.widthsOfExtras;
//
//		int maxExpectedAvailWidth = availWidth - widthsOfExtras;
//
//		
//
//		// Look at columns with absolute sizes
...
//			HtmlLength widthLength = colSizeInfo.htmlLength;
//
//			if(widthLength != null && widthLength.getLengthType() == HtmlLength.PIXELS) {
//
//				//TODO: MULTI-LENGTH not supported
//
//				int actualSizeInt = widthLength.getRawValue();
//
//				if(actualSizeInt < colSizeInfo.minSize) {
//
//					actualSizeInt = colSizeInfo.minSize;
//
//				}
//
//				widthUsedByAbsolute += actualSizeInt;
...
//			}
//
//			else if(widthLength != null && widthLength.getLengthType() == HtmlLength.LENGTH) {
//
//				percentSum += widthLength.getRawValue();
//
//			}
//
//		}
...
//			
//
//		// Look at columns with no specified widths		
//
//		int widthUsedByNoSize = 0;
//
//		// Pre-render cells with no specified width
//
//		this.preRenderNoWidth(hasBorder, cellSpacing, maxExpectedAvailWidth);
//
//		
//
//		// Set sizes to max rendered width
//
//		for(int i = 0; i < numCols; i++) {
//
//			SizeInfo colSizeInfo = columnSizes[i];
...
//			if(widthLength == null) {
//
//				int actualSizeInt = colSizeInfo.maxRenderedSize;
//
//				widthUsedByNoSize += actualSizeInt;
//
//				colSizeInfo.actualSize = actualSizeInt;
//
//			}
...
//		
//
//		// Calculate total width assuming current column widths			
//
//		int prelimTotalCellWidth = (int) Math.round((widthUsedByAbsolute + widthUsedByNoSize) / (1 - (percentSum / 100.0)));
//
//		
//
//		// See if those with no specified width are too big and shrink
//
//		if(prelimTotalCellWidth > maxExpectedAvailWidth) {
//
//			int diff = prelimTotalCellWidth - maxExpectedAvailWidth;
//
//			int shrunkWidthByNoSize = widthUsedByNoSize - diff; 
//
//			double ratio = (double) shrunkWidthByNoSize / widthUsedByNoSize;
//
//			widthUsedByNoSize = 0;
//
//			for(int i = 0; i < numCols; i++) {
...
//				if(widthLength == null) {
//
//					int actualSize = colSizeInfo.actualSize;
//
//					actualSize = (int) Math.round(actualSize * ratio);
//
//					if(actualSize < colSizeInfo.minSize) {
//
//						actualSize = colSizeInfo.minSize;
//
//					}
//
//					colSizeInfo.actualSize = actualSize;
//
//					widthUsedByNoSize += actualSize;
//
//				}
//
//			}				
...
//
//		int expectedTotalCellWidth = (int) Math.round((widthUsedByAbsolute + widthUsedByNoSize) / (1 - (percentSum / 100.0)));
//
//		
//
...
//			if(widthLength != null && widthLength.getLengthType() == HtmlLength.LENGTH) {
//
//				int actualSizeInt = widthLength.getLength(expectedTotalCellWidth);
//
//				if(actualSizeInt < colSizeInfo.minSize) {
//
//					actualSizeInt = colSizeInfo.minSize;
...
//		// Set width of table
//
//		this.tableWidth = widthUsedByAbsolute + widthUsedByNoSize + widthUsedByPercent + widthsOfExtras;
//
//		
//
//		// Pre-render each cell based on computed widths - this sets min heights		
//
//		this.preRender(hasBorder, cellSpacingX, cellSpacingY, false);
//
//	}
...
//	private void reduceTableWidthForContent(int cellAvailWidth, int widthsOfExtras) {
//		SizeInfo[] colSizes = this.columnSizes;
//		int numColumns = colSizes.length;
//		// Find the smallest actual width
//		int minActualWidth = -1;
//		SizeInfo minActualWidthSizeInfo = null; 
//		for(int i = 0; i < numColumns; i++) {
//			SizeInfo colSize = colSizes[i];
//			int actualWidth = colSize.actualSize.intValue();
//			HtmlLength widthLength = colSize.htmlLength;
//			if((widthLength == null || widthLength.getLengthType() != HtmlLength.PIXELS) && (minActualWidth == -1 || minActualWidth > actualWidth)) {
//				minActualWidth = actualWidth;
//				minActualWidthSizeInfo = colSize; 
//			}
//		}
//		if(minActualWidthSizeInfo != null) {
//			int maxRenderedSize = minActualWidthSizeInfo.maxRenderedSize;
//			if(maxRenderedSize < minActualWidth) {
//				double ratio = (double) maxRenderedSize / minActualWidth;
//				int totalUsed = 0;
//				for(int i = 0; i < numColumns; i++) {
//					SizeInfo colSize = colSizes[i];
//					HtmlLength widthLength = colSize.htmlLength;
//					int newActualSize;
//					if(widthLength == null || widthLength.getLengthType() != HtmlLength.PIXELS) {
//						newActualSize = (int) Math.round(colSize.actualSize.intValue() * ratio);
//						if(newActualSize < colSize.maxRenderedSize) {
//							newActualSize = colSize.maxRenderedSize;
//						}
//						colSize.actualSize = newActualSize;
//					}
//					else {
//						newActualSize = colSize.actualSize.intValue();
//					}
//					totalUsed += newActualSize;
//				}
//				this.tableWidth = totalUsed + widthsOfExtras;
//			}
//		}
//	}
//
...
//	 * It sets minimum row heights based on this.
//	 */
//	private final void preRenderNoWidth(int hasBorder, int cellSpacing, int maxExpectedAvailWidth) {
//		SizeInfo[] colSizes = this.columnSizes;
//		ArrayList allCells = this.ALL_CELLS;
//		int numCells = allCells.size();
//		for(int i = 0; i < numCells; i++) {
...
//			}
//			else {
//				hasNoWidth = colSizes[col].htmlLength == null;
//			}
//			if(hasNoWidth) {
//				cell.layoutFor(maxExpectedAvailWidth, 0);
//			}
//		}
//
//	}
//
	 ...
	 */
	public final void doLayout(Container container) {
	...
	public final void paintDecorations(Graphics g, Dimension size, Insets insets) {
		int border = this.border;
		if(border > 0) {
			// Paint table border
			...
			// Paint border of each cell
			
			ArrayList allCells = this.ALL_CELLS;
			int numCells = allCells.size();
			for(int i = 0; i < numCells; i++) {
				ActualCell cell = (ActualCell) allCells.get(i);
				java.awt.Component panel = cell.getComponent();
